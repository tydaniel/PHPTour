...
class ModuleMenu extends Admin_Controller {
    ...
    function add($parent_id = 0) {
        if (isset($_POST['parent_id']) && is_ajax()) {
            ...
            if ($menu_name == "")
                exit(json_encode(array('status' => false, 'tips' => ' 菜单名称不能为空')));
            ...
            if ($status) {
                if ($depth <= 1)
                    $this->Module_menu_model->update(array('method' => 'go_' . $status), array('menu_id' => $status));
                $this->repair();
                $this->cache();
    ...
    private function get_arrparentid($catid, $arrparentid = '', $n = 1) {
        if ($n > 5 || !is_array($this->menus) || !isset($this->menus[$catid]))
            return false;
    ...
    private function repair() {
        ...
        if (is_array($this->menus)) {
            ...
            foreach ($this->menus as $cid => $cat) {
                ...
                if ($categorys[$cid]['list_order'] != $listorder)
                    $this->Module_menu_model->update(array('list_order' => $listorder), array('menu_id' => $cid));
            }
        }
