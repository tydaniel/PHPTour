...
<?php if ($data_list): ?>
        ...
        <div class="table-responsive">
            <table class="table table-hover dataTable"> 
                <thead>
                    <tr>
                        <th>#</th>
                        <?php unset($fields_caption[0]);
                        foreach ($fields_caption as $k => $v): ?>
    ...
    <?php endforeach; ?>
        ...
        <?php $new_fields = $fields;
        $first_key = $new_fields[0];
        $first_value = $v[$first_key]; ?>
                        <tr>
                            <td><?php echo $k + 1 ?></td>
        <?php unset($new_fields[0]);
        $caption = array();
        foreach ($new_fields as $kk => $vv): if (in_array($vv, $fields_convert)) $caption[] = $v[$vv]; ?>
                                <td><?php echo $v[$vv] ?></td>
        <?php endforeach; ?>
