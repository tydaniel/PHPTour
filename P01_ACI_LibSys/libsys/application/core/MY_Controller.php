...
class MY_Controller extends CI_Controller {
    ...
    function _check_module() {
        ...
        if ($_aci_config == NULL)
            exit($this->page_data['controller_name'] . '模块:未在ACI中注册');
...
class Member_Controller extends Front_Controller {
    ...
    protected function check_priv() {
        ...
        if ($this->group_id == SUPERADMIN_GROUP_ID)
            return true;
        if (preg_match('/^public_/', strtolower($this->page_data['method_name'])))
            return true;
    ...
    protected function nav_menu($parent_id, $with_self = 0, $show_where = 0) {
        ...
        if ($this->all_module_menu)
            foreach ($this->all_module_menu as $k => $v) {
                if ($show_where == 1 && strtolower($v['folder']) != "adminpanel")
                    continue;
                if ($show_where == 0 && strtolower($v['folder']) == "adminpanel")
                    continue;
...
class Admin_Controller extends Member_Controller {
    ...
    protected function check_priv() {
        if ($this->page_data['folder_name'] == 'adminpanel' && $this->page_data['controller_name'] == 'manage' && in_array($this->page_data['method_name'], array('login', 'logout', 'manage')))
            return true;
        if ($this->group_id == SUPERADMIN_GROUP_ID)
            return true;
        if (preg_match('/^public_/', strtolower($this->page_data['method_name'])) || (strtolower($this->page_data['method_name']) == "go" && strtolower($this->page_data['controller_name']) == "manage"))
            return true;
